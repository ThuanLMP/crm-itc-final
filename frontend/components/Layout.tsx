...
import { Users, BarChart3, LogOut, Settings, Database, Calendar, Menu, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export function Layout({ children }: LayoutProps) {
  ...
  const NavItems = ({ mobile = false, collapsed = false, onItemClick }: { mobile?: boolean; collapsed?: boolean; onItemClick?: () => void }) => (
    <>
      {navigation.map((item) => {
        ...
        return (
          <Link
            ...
            } ${mobile ? "justify-center md:justify-start" : ""} ${collapsed ? "justify-center" : ""}`}
            title={collapsed ? item.name : undefined}
          >
            <Icon className={`${mobile || collapsed ? "h-6 w-6" : "mr-3 h-5 w-5"} ${
              isActive ? "text-white" : "text-slate-500 group-hover:text-slate-700"
            }`} />
            {!mobile && !collapsed && <span className="ml-3">{item.name}</span>}
            {(mobile || collapsed) && <span className="sr-only">{item.name}</span>}
          </Link>
        );
      })}
    </>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      <div className="flex h-screen">
        {/* Desktop Sidebar */}
        <div className={`hidden lg:flex lg:flex-col transition-all duration-300 ${isSidebarCollapsed ? 'lg:w-20' : 'lg:w-64'}`}>
          <div className="flex flex-col flex-grow pt-5 bg-white/80 backdrop-blur-sm border-r border-slate-200 shadow-sm overflow-y-auto">
            <div className="flex items-center flex-shrink-0 px-6 relative">
              ...
              {isSidebarCollapsed && (
                <div className="p-2 bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg mx-auto">
                  <Users className="h-6 w-6 text-white" />
                </div>
              )}
              <Button
                ...
                className="absolute -right-3 top-1/2 -translate-y-1/2 h-6 w-6 p-0 bg-white border border-slate-200 hover:bg-slate-50 shadow-sm rounded-full"
              >
                {isSidebarCollapsed ? (
            ...
            <div className={`flex-shrink-0 flex border-t border-slate-200 p-4 bg-slate-50/50 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
              {!isSidebarCollapsed ? (
